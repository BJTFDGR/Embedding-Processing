'''
predict salad in the private dataset
7000 16250 15 99
'''
...
class MLP(torch.nn.Module):
    def __init__(self, n_feature=768, n_hidden=512, n_output=2, dropout=0.5):
        super(MLP, self).__init__()
        ...
        self.hidden_3 = torch.nn.Linear(
            n_hidden//2, n_hidden//4)  # hidden layer
        self.bn3 = torch.nn.BatchNorm1d(n_hidden//4)

        self.hidden_4 = torch.nn.Linear(
            n_hidden // 4, n_hidden // 8)  # hidden layer
...
weight_CE = torch.FloatTensor([1, 1.025])

# loss func and optim
optimizer = torch.optim.Adam(model.parameters(), lr=0.0101, betas=(
...
def AccuarcyComputeT3(pred, label):
    ...
    #     print(pred.shape(),label.shape())
        ...
        count = 0
        values, indices = torch.tensor(pred).topk(
            3, dim=1, largest=True, sorted=True)
        for i in range(indices.shape[0]):
            if label[i] in indices[i]:
                count += 1
        ...
        for i, data in enumerate(train_dataset):
            ...
            with torch.no_grad():
                if (i+1) == len(train_dataset):
                    ...
                    print(epoch, ":", acc)
                if acc > 0.75:
                    print(inputs, labels, loss, np.argmax(
                        outputs.cpu().data.numpy(), 1))

                mid = AccuarcyCompute(outputs, labels)
                if mid > 0.7:
                    with open('5_layer_top_3_training_data.txt', 'a+') as f:
                        print("MID>0.7", mid, np.argmax(
                            outputs.cpu().data.numpy(), 1), labels)
...
plt.savefig('onlySaladBinary_5_layer_top_1_test_data_list_epoch.png')
plt.close()
plt.plot(range(len(training_data_list_epoch)), training_data_list_epoch)
plt.savefig('onlySaladBinary_5_layer_top_1_training_data_list_epoch.png')
plt.close()
...
plt.savefig('onlySaladBinary_5_layer_top_1_test_data_list_sample.png')
plt.close()
plt.hist(training_data_list_sample, range=(0, 1))
plt.xlabel("label")
plt.ylabel("freq")
plt.xlim(0, 1)
plt.title("Binary_5_layer_top_1_training_data_list_sample")
plt.savefig('onlySaladBinary_5_layer_top_1_training_data_list_sample.png')
plt.close()
print(len(training_data_list_sample))
print(len(test_data_list_sample))
print(sum(i > 0.7 for i in training_data_list_sample))
# 7000 16250 19 78
k1, k2 = [], []
...
plt.xlim(0, 0.6)
plt.title("Binary_5_layer_top_1_test_data_list_sample")
plt.savefig('onlySaladF02_Binary_5_layer_top_1_training_data_list_sample.png')
plt.close()
...
plt.xlim(0.6, 0.9)
plt.title("Binary_5_layer_top_1_test_data_list_sample")
plt.savefig('onlySaladF28_Binary_5_layer_top_1_training_data_list_sample.png')
plt.close()
...
plt.xlim(0, 0.6)
plt.title("Binary_5_layer_top_1_test_data_list_sample")
plt.savefig('onlySaladF02_Binary_5_layer_top_1_test_data_list_sample.png')
plt.close()
...
plt.xlim(0.6, 0.9)
plt.title("Binary_5_layer_top_1_test_data_list_sample")
plt.savefig('onlySalad_F28_Binary_5_layer_top_1_test_data_list_sample.png')
plt.close()
